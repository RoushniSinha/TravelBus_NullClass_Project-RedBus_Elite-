  ...
  useEffect(() => {
    const fetchBuses = async () => {
      ...
      try {
        ...
        const busList = querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
        setBuses(busList);
      } catch (error) {
        ...
        <aside className="w-full md:w-64 space-y-8">
          <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
            ...
            <div className="space-y-6">
              <div>
                <label className="text-sm font-bold text-slate-700 block mb-3">Bus Type</label>
                <div className="space-y-2">
                  {['AC Sleeper', 'Non-AC Sleeper', 'AC Seater', 'Luxury'].map(type => (
                    <label key={type} className="flex items-center gap-3 text-slate-600 cursor-pointer hover:text-blue-600">
              ...
              <div>
                <label className="text-sm font-bold text-slate-700 block mb-3">Departure Time</label>
                <div className="space-y-2">
                  {['Morning (6am-12pm)', 'Afternoon (12pm-6pm)', 'Evening (6pm-12am)', 'Night (12am-6am)'].map(time => (
                    <label key={time} className="flex items-center gap-3 text-slate-600 cursor-pointer hover:text-blue-600">
                      <input type="checkbox" className="rounded border-slate-300 text-blue-600 focus:ring-blue-500" />
                  ...
                  <div className="flex flex-col lg:flex-row justify-between gap-8">
                    <div className="flex-grow space-y-4">
                      <div className="flex items-center gap-3">
                        <h3 className="text-xl font-bold text-slate-900">{bus.busType}</h3>
                      ...
                      <div className="flex items-center gap-8">
                        <div className="space-y-1">
                          <div className="text-2xl font-bold text-slate-900">{new Date(bus.departureTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</div>
                        ...
                        <div className="flex-grow flex items-center gap-2">
                          <div className="h-px bg-slate-200 flex-grow relative">
                            <div className="absolute -top-1.5 left-1/2 -translate-x-1/2 bg-white px-2 text-[10px] font-bold text-slate-400 uppercase">8h 30m</div>
                          </div>
                        </div>
                        <div className="space-y-1 text-right">
                          <div className="text-2xl font-bold text-slate-900">{new Date(bus.arrivalTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</div>
                          <div className="text-xs font-bold text-slate-400 uppercase">{bus.toCity}</div>
                        </div>
                      </div>
                    </div>

                    <div className="lg:w-48 flex flex-col justify-between items-end border-t lg:border-t-0 lg:border-l border-slate-100 pt-6 lg:pt-0 lg:pl-8">
                      <div className="text-right">
                        <div className="text-xs font-bold text-slate-400 uppercase">Starting from</div>
                        <div className="text-3xl font-bold text-slate-900">₹{bus.price}</div>
                      </div>
                      <Link 
                        to={`/bus/${bus.id}`}
                ...
                <div className="bg-slate-50 px-6 py-3 flex justify-between items-center border-t border-slate-100">
                  <div className="flex gap-4">
                    {bus.amenities?.slice(0, 3).map((a: string) => (
                      <div key={a} className="flex items-center gap-1.5 text-xs font-bold text-slate-500">
                        <Info className="w-3 h-3" /> {a}
                      </div>
                    ))}
                  </div>
                  <div className="text-xs font-bold text-blue-600">{bus.availableSeats} Seats Left</div>
                </div>
              </motion.div>
            ))
          )}
