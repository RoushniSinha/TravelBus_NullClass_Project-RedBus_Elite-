...
import { Bus, Bell, User, LogOut, Menu, X, Navigation, Sun, Moon, Globe, Check, Shield, LayoutDashboard } from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-20">
          <div className="flex items-center">
            <Link to="/" className="flex items-center gap-2 group">
              ...
              <span className="font-black text-2xl text-slate-900 dark:text-white tracking-tighter">
                Travel<span className="text-blue-600">Bus</span>
              </span>
            </Link>
          </div>
          ...
          <div className="hidden md:flex items-center space-x-8">
            <Link to="/search" className="text-sm font-bold text-slate-600 dark:text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 transition-colors">{t('nav.search')}</Link>
            <Link to="/live-tracking" className="text-sm font-bold text-slate-600 dark:text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 transition-colors flex items-center gap-1">
              <Navigation className="w-4 h-4" /> {t('nav.live_tracking')}
            </Link>
            <Link to="/stories" className="text-sm font-bold text-slate-600 dark:text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 transition-colors">{t('nav.stories')}</Link>
                  ...
                  >
                    {languages.map((lang) => (
                      ...
                      >
                        ...
                        {i18n.language === lang.code && <Check className="w-4 h-4 text-blue-600" />}
                      </button>
                    ))}
                  </motion.div>
                )}
            ...
            {user ? (
              <div className="flex items-center space-x-4">
                ...
                <div className="relative group">
                  <button className="flex items-center gap-3 bg-slate-50 dark:bg-slate-800 p-1.5 pr-4 rounded-full hover:bg-slate-100 dark:hover:bg-slate-700 transition-all">
                    ...
                    <span className="text-sm font-bold text-slate-700 dark:text-slate-200">{user.displayName?.split(' ')[0] || 'User'}</span>
                  </button>
                  <div className="absolute right-0 mt-2 w-56 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl shadow-xl py-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all transform origin-top-right scale-95 group-hover:scale-100">
                    ...
                    {role === 'admin' && (
                      <Link to="/admin" className="flex items-center space-x-3 px-4 py-3 text-sm font-bold text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/20">
              ...
              <div className="flex items-center space-x-4">
                <Link to="/login" className="text-sm font-bold text-slate-600 dark:text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 transition-colors">{t('nav.login')}</Link>
                <Link to="/register" className="bg-blue-600 text-white px-6 py-2.5 rounded-xl text-sm font-bold hover:bg-blue-700 transition-all shadow-lg shadow-blue-900/10">
                  {t('nav.register')}
                </Link>
              </div>
            )}
        ...
        {isOpen && (
          ...
          >
            <div className="px-4 pt-4 pb-8 space-y-2">
              <Link to="/search" className="block px-4 py-3 text-base font-bold text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-xl">{t('nav.search')}</Link>
              <Link to="/stories" className="block px-4 py-3 text-base font-bold text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-xl">{t('nav.stories')}</Link>
              <Link to="/live-tracking" className="block px-4 py-3 text-base font-bold text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-xl">{t('nav.live_tracking')}</Link>
              ...
              {user ? (
                <>
                  <div className="h-px bg-slate-100 dark:bg-slate-800 my-4"></div>
                  <Link to="/profile" className="block px-4 py-3 text-base font-bold text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-xl">{t('nav.profile')}</Link>
                  <Link to="/my-bookings" className="block px-4 py-3 text-base font-bold text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-xl">{t('nav.my_bookings')}</Link>
                  <button onClick={handleLogout} className="w-full text-left px-4 py-3 text-base font-bold text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-xl">{t('nav.logout')}</button>
                </>
              ) : (
                <div className="grid grid-cols-2 gap-4 pt-4">
                  <Link to="/login" className="flex items-center justify-center px-4 py-3 text-base font-bold text-slate-600 dark:text-slate-300 bg-slate-50 dark:bg-slate-800 rounded-xl">{t('nav.login')}</Link>
                  <Link to="/register" className="flex items-center justify-center px-4 py-3 text-base font-bold text-white bg-blue-600 rounded-xl">{t('nav.register')}</Link>
                </div>
              )}
            </div>
          </motion.div>
