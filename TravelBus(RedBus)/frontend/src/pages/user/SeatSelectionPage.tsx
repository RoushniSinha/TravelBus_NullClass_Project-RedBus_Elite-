      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="text-center">
            <h1 className="text-2xl font-bold text-slate-900">{bus.operatorName}</h1>
            <p className="text-slate-500 text-sm font-medium">{bus.fromCity} → {bus.toCity}</p>
          </div>
          ...
          <div className="flex-grow">
            <div className="bg-white p-10 rounded-3xl border border-slate-200 shadow-xl relative overflow-hidden">
              ...
              <div className="max-w-xs mx-auto space-y-10">
                {/* Driver Section */}
                <div className="flex justify-between items-center px-4 py-2 bg-slate-50 rounded-xl border border-slate-200 mb-8">
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Front</span>
                  ...
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Driver</span>
                </div>
                          ...
                          className={`w-12 h-12 rounded-xl border transition-all flex items-center justify-center text-[10px] font-bold relative group ${
                            isBooked 
                              ...
                              : isLockedByOthers
                                ...
                                : isLockedByMe
                                  ? 'bg-red-600 border-red-600 text-white shadow-lg shadow-red-900/20 scale-110'
                                  : 'bg-slate-100 border-slate-200 text-slate-500 hover:border-red-600 hover:scale-105'
                          }`}
                        >
                          <div className="absolute top-1 left-1.5 text-[8px] opacity-50">{seat.id}</div>
                          <User className={`w-5 h-5 ${isLockedByMe ? 'text-white' : 'text-slate-400 group-hover:text-red-600'}`} />
                          {isLockedByOthers && (
                            <div className="absolute -top-1 -right-1 w-2.5 h-2.5 bg-amber-500 rounded-full border-2 border-white animate-pulse"></div>
                          )}
                        </button>
                      </div>
                    );
            ...
            <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-xl space-y-8 sticky top-24">
              ...
              <div className="space-y-4">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="text-slate-900 font-bold">{selectedSeats.length > 0 ? selectedSeats.join(', ') : 'None'}</span>
                </div>
                <div className="flex justify-between text-sm">
              ...
              <button
                ...
                className="w-full bg-red-600 text-white py-4 rounded-xl font-bold hover:bg-red-700 transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-red-900/10"
              >
