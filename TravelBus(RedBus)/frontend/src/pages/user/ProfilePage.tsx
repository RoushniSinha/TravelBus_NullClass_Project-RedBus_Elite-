...
export default function ProfilePage() {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-12 space-y-12 bg-slate-50 min-h-screen transition-colors">
      <div className="flex flex-col lg:flex-row gap-12">
        {/* Sidebar */}
        <aside className="w-full lg:w-80 space-y-8">
          <div className="bg-white p-8 rounded-[40px] border border-slate-200 shadow-sm text-center space-y-6 transition-colors">
            <div className="relative inline-block">
              <div className="w-32 h-32 bg-blue-100 rounded-full flex items-center justify-center text-blue-600 text-4xl font-bold border-4 border-white shadow-lg overflow-hidden">
                {user?.avatar ? (
              ...
              <label className="absolute bottom-0 right-0 w-10 h-10 bg-blue-600 text-white rounded-full flex items-center justify-center border-4 border-white hover:bg-blue-700 transition-all cursor-pointer">
          ...
          <nav className="bg-white p-4 rounded-3xl border border-slate-200 shadow-sm space-y-1 transition-colors">
            {tabs.map((tab) => {
              const Icon = tab.icon;
              return (
                <button 
        ...
        <div className="flex-grow space-y-8">
          <AnimatePresence mode="wait">
            <motion.div
              ...
              exit={{ opacity: 0, x: -20 }}
              className="bg-white p-10 rounded-[40px] border border-slate-200 shadow-sm transition-colors"
            >
              {activeTab === 'personal' && (
                <div className="space-y-8">
                  <div className="flex justify-between items-center">
                    <h3 className="text-2xl font-bold text-slate-900">{t('profile.personal_info')}</h3>
                    {!editing && (
                      <button 
                  ...
                  <form onSubmit={handleUpdate} className="space-y-8">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                      <div className="space-y-2">
                        <label className="text-xs font-bold text-slate-500 uppercase tracking-widest ml-1">{t('profile.full_name')}</label>
                        <div className="relative">
                          <UserIcon className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 w-5 h-5" />
                          <input 
                            type="text" 
                            disabled={!editing}
                            className="w-full pl-12 pr-4 py-4 bg-slate-50 border border-slate-200 rounded-2xl text-slate-900 focus:ring-2 focus:ring-blue-500 outline-none disabled:opacity-60 transition-all"
                            value={formData.name}
                      ...
                      <div className="space-y-2">
                        <label className="text-xs font-bold text-slate-500 uppercase tracking-widest ml-1">{t('profile.email')}</label>
                        <div className="relative">
                          <Mail className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 w-5 h-5" />
                          <input 
                            type="email" 
                            disabled
                            className="w-full pl-12 pr-4 py-4 bg-slate-50 border border-slate-200 rounded-2xl text-slate-900 opacity-60 cursor-not-allowed"
                            value={user?.email || ''}
                          />
                        </div>
                      </div>
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="relative">
                          ...
                          <select 
                            disabled={!editing}
                            className="w-full pl-12 pr-4 py-4 bg-slate-50 border border-slate-200 rounded-2xl text-slate-900 focus:ring-2 focus:ring-blue-500 outline-none disabled:opacity-60 transition-all appearance-none"
              ...
              {activeTab === 'notifications' && (
                <div className="space-y-8">
                  <h3 className="text-2xl font-bold text-slate-900">{t('profile.notifications')}</h3>
                      ...
                      <div key={pref.id} className="flex items-center justify-between p-6 bg-slate-50 rounded-3xl border border-slate-100">
                        ...
                        <button 
                          ...
                          className={`w-14 h-8 rounded-full relative transition-all ${
                            formData.notificationPreferences[pref.id as keyof typeof formData.notificationPreferences] ? 'bg-blue-600' : 'bg-slate-200'
                          }`}
                        >
              ...
              {activeTab === 'security' && (
                <div className="space-y-10">
                  <div className="space-y-6">
                    <h3 className="text-2xl font-bold text-slate-900">{t('profile.security')}</h3>
                    ...
                    <div className="p-8 bg-slate-50 rounded-[32px] border border-slate-100 space-y-6">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-2">
                          ...
                          <input type="password" placeholder="••••••••" className="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-blue-500" />
                        </div>
                        <div className="space-y-2">
                      ...
                      <button className="bg-slate-900 text-white px-6 py-3 rounded-xl font-bold hover:bg-blue-600 hover:text-white transition-all">
                        Update Password
                      </button>
                    </div>
                    ...
                    <div className="p-8 bg-slate-50 rounded-[32px] border border-slate-100 space-y-4">
                      ...
                      <div className="flex items-center justify-between py-3 border-b border-slate-100">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <div className="font-bold text-slate-900">Chrome on MacOS</div>
                            <div className="text-xs text-slate-500">Mumbai, India • Active now</div>
                          </div>
                        </div>
                        <div className="text-xs font-bold text-green-600 bg-green-50 px-3 py-1 rounded-full">Current</div>
                      </div>
                    </div>
              ...
              {activeTab === 'reviews' && (
                <div className="space-y-8">
                  <h3 className="text-2xl font-bold text-slate-900">{t('profile.my_reviews')}</h3>
                  {reviews.length === 0 ? (
                    <div className="text-center py-20 space-y-4">
                      <div className="w-20 h-20 bg-slate-50 rounded-full flex items-center justify-center mx-auto">
                        <Star className="w-10 h-10 text-slate-300" />
                      </div>
                      <p className="text-slate-500 font-medium">You haven't shared any travel experiences yet.</p>
                    </div>
                  ) : (
                    <div className="grid grid-cols-1 gap-6">
                      {reviews.map((review) => (
                        <div key={review.id} className="p-8 bg-slate-50 rounded-[32px] border border-slate-100 space-y-4">
                          <div className="flex justify-between items-start">
                            <div className="space-y-1">
                              <div className="font-bold text-slate-900">{review.busName || 'Elite Express'}</div>
