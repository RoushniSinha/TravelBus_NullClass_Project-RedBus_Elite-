...
export default function AdminStoriesPage() {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-3xl font-bold text-slate-900 dark:text-white tracking-tight">Travel Stories</h1>
        <p className="text-slate-500 dark:text-slate-400 font-medium">Moderate and manage community travel stories.</p>
      </div>
          ...
          >
            ...
            <div className="p-6 flex-grow space-y-4">
              <div className="flex justify-between items-start">
                <div>
                  <h3 className="text-xl font-bold text-slate-900 dark:text-white line-clamp-1">{story.title}</h3>
                  <p className="text-sm text-slate-500 dark:text-slate-400">By {story.authorName}</p>
                </div>
              ...
              <div className="flex gap-2 pt-4 border-t border-slate-50 dark:border-slate-800">
                <button 
                  ...
                  className={`flex-grow py-2 rounded-xl text-xs font-bold uppercase tracking-widest transition-all ${
                    story.isApproved ? 'bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400' : 'bg-green-50 dark:bg-green-900/20 text-green-600 dark:text-green-400'
                  }`}
                >
